...
def test_install_delete_single_plugin_string(jenkins):
    ...
    jenkins.delete_plugin(
        plugin_name, force_restart=True, wait_for_reboot=True
    )
...
def test_install_delete_multiple_plugins_mix_string_object(jenkins):
    ...
    jenkins.install_plugins(
        [plugin_one, plugin_two], restart=True, wait_for_reboot=True
    )

    assert (plugin_one_name and plugin_two_name) in jenkins.plugins

    # Force restart so plugins are deleted from plugin-list
    jenkins.delete_plugins(
        [plugin_one_name, plugin_two], force_restart=True, wait_for_reboot=True
    )
